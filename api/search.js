export default async function handler(req, res) {
  ...
  try {
    // 네이버 API는 한 번에 최대 100개까지만 가능하므로 2번 호출
    const fetchResults = async (start) => {
      const response = await fetch(
        `https://openapi.naver.com/v1/search/shop.json?query=${encodeURIComponent(query)}&display=100&start=${start}&sort=asc`,
        {
          headers: {
    ...
    // 첫 번째 100개
    const data1 = await fetchResults(1);
    
    // 두 번째 100개 (101~200)
    const data2 = await fetchResults(101);
    ...
    // 판매가 기준 정렬
    allItems.sort((a, b) => parseInt(a.lprice) - parseInt(b.lprice));
