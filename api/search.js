export default async function handler(req, res) {
  ...
  const fashionBrands = [
    ...
    '디키즈', 'dickies', '챔피온', 'champion', '캉골', 'kangol', '스톤아일랜드', 'stone island',
  ...
  try {
    ...
    if (data.items) {
      ...
      data.items = data.items.map(item => {
        ...
        // 해외배송 판단 로직
        const isOverseas = 
          item.productType === '2' || // 네이버 API에서 해외상품 타입
          mallName.includes('해외') ||
          mallName.includes('직구') ||
          mallName.includes('global') ||
          mallName.includes('amazon') ||
          mallName.includes('ebay') ||
          title.includes('해외') ||
          title.includes('직구') ||
          title.includes('관부가세') ||
          title.includes('병행수입');
